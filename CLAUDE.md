...
## Avatar
- SVG 2D en style figurine à blocs (demande de Théo, 2026-09-26, d'après des exemples de minifigurines ; dessin original, sans marque) : tête cylindrique à plot avec yeux et sourire (traits clairs sur peau foncée), torse en trapèze, mains en pince, hanches et jambes en blocs, reflets clairs. Relief simulé (demandes du 2026-09-26) : lumière venant du haut à gauche, épaisseur de chaque pièce (tranche sombre décalée vers la droite et le bas), calques d'ombrage translucides (blocs, cylindres pour la tête et la casquette, sphères pour les mains), ombres de contact entre les pièces, ombres des pans ouverts sur la couche du dessous, ombre portée et ombre au sol. Peau à la teinte MST choisie (même si le teint est inactif dans les combinaisons), une zone par type ; les vêtements ouverts sont « imprimés » sur le torse.
...
- Pièce couverte : dessinée avec le hex du vêtement retenu (plus petit ΔE00, puis le plus ancien). Joker : noir avant blanc. Pièce en manque : dessinée dans la couleur manquante (noir pour un joker), avec un petit panneau ⚠ posé dessus ; le détail et le résumé de la proposition l'annoncent aussi par ⚠ (demande de Théo, 2026-09-26, à la place des hachures).
...
4. Manques fréquents : bilan (nombre de propositions et de tenues types), top 10 (rang, pastille, nom, type, nombre de propositions), dépliant « Tenues prises en compte » avec un bouton « Retirer » par tenue, après confirmation (demande de Théo, 2026-09-26 ; la tenue revient si on la redemande). Sans tenue type : invitation et bouton vers l'onglet Tenue ; aucune proposition retenue ou rien ne manque : message dédié. Le calcul (js/statistiques.js, MANQUES_FREQUENTS_MAX = 10) suit l'affichage de « Calcul des manques… » et reste en mémoire tant que garde-robe, réglages, tenues types et catalogue ne changent pas. La pastille du joker (moitié #000000, moitié #ffffff) sert aussi au détail des propositions.
...
- Service worker à la racine (./sw.js), cache versionné, précache avec { cache: 'reload' } ; data/papier-tigre.json, tests/ et outils/ jamais précachés ; bandeau « Nouvelle version, recharger ».
  Réalisé (étape 8) : cache « garde-robe-<VERSION> », VERSION de sw.js égale à VERSION_APP (à changer ensemble à chaque envoi, sinon l'iPhone garde l'ancienne version) ; liste FICHIERS comparée par les tests aux dossiers js/, js/ecrans/, css/ et icones/. Cache d'abord puis réseau ; pour une page, paramètres ignorés. À l'activation, seuls les anciens caches préfixés « garde-robe- » sont supprimés (origine partagée). La nouvelle version attend : bandeau « Nouvelle version disponible » (js/mise-a-jour.js), activée seulement quand on touche « Recharger » ; Réglages > « Charger la dernière version » la cherche et l'active tout de suite. Vérification aussi au retour de l'app au premier plan. Aucun service worker sur localhost et 127.0.0.1 (il servirait d'anciens fichiers pendant le développement).
- Icônes : apple-touch-icon PNG 180×180 (prioritaire sur le manifest), icônes du manifest 192, 512 et 1024 (purpose any) ; display: standalone.
  Réalisé (étape 8, accord de Théo le 2026-09-26) : la figurine de l'avatar en buste, peau MST 5, pull à la première couleur de la combinaison Wada n° 155, sur des bandes de ses deux autres couleurs ; carré plein (iOS arrondit les coins et remplit la transparence en noir). Générées par outils/generer-icones.ps1 depuis outils/icones.html (?apercu : toutes les combinaisons de 3 couleurs) dans icones/.
...
- Tests : Node est absent de la machine. Page tests/tests.html, lancée par tests/lancer-tests.ps1 (serveur outils/serveur.py sur 127.0.0.1, Edge sans fenêtre avec --virtual-time-budget et --dump-dom, ligne « RESULTAT ok/total », code de sortie non nul en cas d'échec). Mesures de temps dans outils/bench.html, car le mode sans fenêtre virtualise le temps. Edge est lancé avec une caméra simulée (--use-fake-device-for-media-stream, --use-fake-ui-for-media-stream, sans torche). Le temps virtuel ne compte ni les décodages d'image ni l'ouverture de la caméra : ces attentes passent par tests/aides.js (attendreReel, avecTempsReel), qui interrogent /__attente (outils/serveur.py) pour laisser s'écouler du temps réel. createImageBitmap ne se termine jamais dans ce mode : les photos sont décodées par <img>. Le temps virtuel n'attend pas non plus les promesses de caches et de navigator.serviceWorker : elles passent par avecTempsReel. Le profil d'Edge est conservé d'un lancement à l'autre : tests/tests.html retire d'abord tout service worker et cache « garde-robe- » restant, et pwa.test.js (en dernier) retire le sien. Chaque test est borné à 20 s et la page affiche sa progression.
- Git : un commit par étape terminée, envoyé sur https://github.com/theomando/garde-robe (dépôt public depuis le 2026-09-24, adresse d'auteur privée 186628105+theomando@users.noreply.github.com). Site GitHub Pages activé le 2026-09-26 à la demande de Théo, pour travailler directement sur l'iPhone : https://theomando.github.io/garde-robe/ (vérifié : pages en 200, data/papier-tigre.json et notes en 404). Chaque envoi met le site à jour ; bouton Réglages > « Charger la dernière version » contre le cache de 10 min. Service worker, manifest et icônes ajoutés à l'étape 8 (feu vert de Théo pour GitHub Pages le 2026-09-26). Une app ajoutée à l'écran d'accueil avant l'étape 8 garde son ancienne icône : la supprimer puis la rajouter depuis Safari, ce qui efface ses données (exporter avant).
